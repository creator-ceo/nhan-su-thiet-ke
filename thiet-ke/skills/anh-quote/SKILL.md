...
Bộ máy nằm trong thư mục skill này, `${CLAUDE_PLUGIN_ROOT}/skills/anh-quote/`: `khung.html` (khung gốc để copy ra kiểu mới), `dung.mjs` (render), `fonts/`, và `kieu-mau/` (sáu kiểu dựng sẵn).
...
Xem `anh-quote/kieu/` có kiểu nào khớp công thức Bước 1 chưa. Có thì dùng lại (đó là lý do có thư viện). Chưa có thì **chép một kiểu dựng sẵn** từ `${CLAUDE_PLUGIN_ROOT}/skills/anh-quote/kieu-mau/` vào `anh-quote/kieu/`, hoặc **copy `khung.html` ra một file kiểu mới** rồi sửa.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/anh-quote/dung.mjs" --kieu anh-quote/kieu/<kiểu>.html --noi-dung anh-quote/ra/<lượt>/noi-dung.json --ra anh-quote/ra/<lượt>/anh.png
```

`${CLAUDE_PLUGIN_ROOT}` là biến Claude Code tự đặt — cứ dán nguyên văn, đừng thay bằng đường dẫn tay.
